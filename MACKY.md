...
dictation is a separate safety-first path, not a shortcut into the assistant. hold **ctrl + fn** over an editable field and Macky first verifies that exact field through local Accessibility. only then does it capture audio. on release it gets one final transcription, optionally formats it, revalidates the same app and field, and inserts it once. it never types partial text, calls MCP or local assistant tools, speaks, presses Return, sends a message, or submits a form.

if focus changes while the user dictates, Macky does not write into the new control. it keeps the final result transiently in the notch and offers an explicit **Copy** button; it never overwrites the clipboard automatically in that recovery path. Terminal is insertion-only: text is staged at the prompt and never executed.

the dictation path uses the already-deployed Azure AI Foundry `gpt-realtime-2.1-mini` model through a separate on-demand Worker proxy. every ctrl + fn interaction opens one authenticated text-only Realtime session after target validation. the session receives PCM16 24 kHz mono while the shortcut is held; release manually commits the audio and asks for one final text response. it has no tools, no MCP access, no audio output, and no assistant-session context. user keyterms (names, domains, companies, and code identifiers) are sent with the session; Macky never sends focused text, selected text, titles, URLs, recipients, or page contents.

dictation has three style modes: **Literal** preserves wording and renders only explicit spoken layout commands, **Clean** performs conservative punctuation and cleanup, and **Smart** applies app-aware polish. all three run inside the same text-only `gpt-realtime-2.1-mini` response, so there is no second Luna request or cross-provider transcript handoff.
