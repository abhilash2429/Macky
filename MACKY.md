...
the development ASR path is AssemblyAI Universal-3.5 Pro Streaming over a new on-demand Worker proxy. the Worker holds the provider key; every ctrl + fn interaction opens one authenticated streaming connection after target validation and explicitly terminates it on release. there is no warm dictation connection because provider billing is based on open WebSocket duration. audio is PCM16 16 kHz mono. user keyterms (names, domains, companies, and code identifiers) are sent with the session; Macky never sends focused text, selected text, titles, URLs, recipients, or page contents.
