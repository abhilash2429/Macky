...
//  (requestMagicLink), receives a `Speed://auth?token=…` link, and opening it routes
//  back through `handleIncomingURL` → `verify`, which stores the session in the
//  Keychain and flips `phase` to `.authenticated`.
//
...
final class AuthManager: ObservableObject {
    ...
    private static let keychainService = "auren.session"
